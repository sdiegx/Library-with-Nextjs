...
interface DocumentsModalProps {
  handlerCreateDocument: (formData: any) => void;
	authors: Author[];
	setShowModalBooks: React.Dispatch<React.SetStateAction<boolean>>;
}

const DocumentsModal: React.FC<DocumentsModalProps> = ({ handlerCreateDocument, authors, setShowModalBooks }) => {
  ...
  <div className="modal-dialog bg-white rounded-lg overflow-hidden w-3/4 max-w-lg">
    <div className="modal-content">
      <div className="modal-header bg-gray-200 p-4">
        <h5 className="modal-title text-xl font-semibold">Agregar Documento</h5>
        <button type="button" onClick={() => setShowModalBooks(false)} className="close text-gray-600 hover:text-gray-800" data-dismiss="modal" aria-label="Close">
      ...
      <div className="modal-form p-4">
        ...
        <div className="flex justify-between mb-4">
          <button onClick={handlerPlusAuthors} className="bg-blue-800 w-1/2 mx-2 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md">+</button>
          <button onClick={handlerMinusAuthors} className="bg-blue-800 w-1/2 mx-2 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md">-</button>
        </div>
        <div>
          {selectedAuthors.length !== 0 &&
            selectedAuthors.filter(selectedAuthor => selectedAuthor.status === true).map((selectedAuthor) => (
              <div key={selectedAuthor.i} className="inline-block relative mb-2">
